...
{
	static public inline var inkVersionCurrent:Int = 21;
	public static inline var inkVersionMinimumCompatible:Int = 21;
	...
	function TryFollowDefaultInvisibleChoice():Bool
	{
		...
		}); // Where (c => c.choicePoint.isInvisibleDefault).ToList();
	...
	function VisitCountForContainer(container:Container):Int
	{
		...
		{
			ErrorThrow("Read count for target ("
				+ container.name
				+ " - on "
	...
	function TurnsSinceForContainer(container:Container):Int
	{
		...
		{
			ErrorThrow("TURNS_SINCE() for target ("
				+ container.name
				+ " - on "
				+ container.debugMetadata
				+ ") unknown. The story may need to be compiled with countAllVisits flag (-c).");
	...
	function NextSequenceShuffleIndex():Int
	{
		...
		for (i in 0...iterationIndex + 1)
		{ // int i = 0; i <= iterationIndex; ++i
	...
	function VariableStateDidChangeEvent(variableName:String, newValueObj:RObject):Void
	{
		...
		if (observers != null)
		{ // _variableObservers.TryGetValue (variableName, out observers)
	...
	function ContinueInternal():String
	{
		...
		{
			if (count++ > 99999)
				throw "Count iteration limit reached";
			...
			if (!state.inStringEvaluation)
			{
				...
				if (stateAtLastNewline != null)
				{
					...
					{ // !currText.Equals(stateAtLastNewline.currentText)
	...
	function IsTruthy(obj:RObject):Bool
	{
		...
		if (Std.is(obj, Value))
		{
			...
			if (Std.is(val, DivertTargetValue))
			{
				...
				ErrorThrow("Shouldn't use a divert target (to "
		...
		else if (Std.is(contentObj, ControlCommand))
		{
			...
			switch (evalCommand.commandType)
			{
				...
				case ControlCommand.CommandType.PopFunction, ControlCommand.CommandType.PopTunnel:
					var popType:Int = evalCommand.commandType == ControlCommand.CommandType.PopFunction ? cast PushPopType.Function : cast PushPopType.Tunnel;
				...
				case ControlCommand.CommandType.EndString:
					...
					state.outputStream.splice(state.outputStream.length - outputCountConsumed,
						outputCountConsumed); // state.outputStream.RemoveRange (state.outputStream.Count - outputCountConsumed, outputCountConsumed);
				...
				case ControlCommand.CommandType.TurnsSince:
					...
					{ // required else because no breka avialable..
	...
	function VisitChangedContainersDueToDivert():Void
	{
		...
		{
			var prevAncestor:Container = Std.is(previousContentObject,
				Container) ? LibUtil.as(previousContentObject, Container) : LibUtil.as(previousContentObject.parent, Container);
	...
	public function CallExternalFunction(funcName:String, numberOfArguments:Int):Void
	{
		...
		if (!foundExternal)
		{
			if (allowExternalFunctionFallbacks)
			{
				...
				Assert.bool(fallbackFunctionContainer != null,
	...
	inline function TryCoerce<T>(value:Dynamic):Dynamic // is this really needed since already returning Dynamic?? Engine/haxe will hopefully coerce if necessary...
	{
		/*
			var casted:T;
			if (value == null)
	...
	public function BindExternalFunction0(funcName:String, func:Void->Dynamic):Void // Func<object> func
	{
		...
		BindExternalFunctionGeneral(funcName, function(args:Array<Dynamic>):Dynamic
		{ // (object[] args) =>
