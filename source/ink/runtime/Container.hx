...
/*  // for the interest of performance, i'd rather inline as below
	...
	// */
enum abstract CountFlags(Int) from Int to Int
{
	public static inline var Visits:Int = 1;
	var Turns:Int = 2;
	var CountStartOnly:Int = 4;
}
...
class Container extends RObject implements INamedContent
{
	...
	function get_namedOnlyContent():Map<String, RObject>
	{
		...
		if (!namedOnlyContent.iterator().hasNext())
		{ // namedOnlyContent.length == 0
	...
	function set_countFlags(value:Int):Int
	{
		...
		if ((flag & CountFlags.Visits) > 0)
			visitsShouldBeCounted = true;
		if ((flag & CountFlags.Turns) > 0)
			turnIndexShouldBeCounted = true;
		if ((flag & CountFlags.CountStartOnly) > 0)
			countingAtStartOnly = true;
	...
	function ContentWithPathComponent(component:Component):RObject
	{
		...
		else
		{
			...
			{ // namedContent.TryGetValue (component.name, out foundContent)
	...
	public function ContentAtPath(path:Path, partialPathLength:Int = -1):RObject
	{
		...
		for (i in 0...partialPathLength)
		{ // int i = 0; i < partialPathLength; ++i
	...
	public function BuildStringOfHierarchy(sb:StringBuf, indentation:Int, pointedObj:RObject)
	{
		...
		{ // int i=0; i<content.Count; ++i
			...
			{ // CONSIDER: equality check here might require IEquals implementation! But this method isn't critical and only sued for dev debugging
				sb.add("  <---");
			}
		...
		for (k in namedContent.keys())
		{
			if (content.indexOf(cast namedContent.get(k)) >= 0)
			{ // content.Contains ((Runtime.Object)objKV.Value)
				continue;
			}
			else
			{
